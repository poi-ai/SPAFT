    ...
    def culc_dv(self, df):
        ...
        try:
            ...
            for minute in [1, 2, 3, 5, 10, 15, 30, 60, 90]:
                ...
                df[f'change_{minute}min_amount'] = (df['close'].shift(-minute) - df['close']).round(2)
                # n分後の株価変化率(%)を1000倍した値 小数点以下は丸める 最小絶対値はTOPIX銘柄の1000円→999.9円(-0.01%x1000=10)
                df[f'change_{minute}min_rate'] = (df[f'change_{minute}min_amount'] / df['close'] * 100000).round(0)
        except Exception as e:
