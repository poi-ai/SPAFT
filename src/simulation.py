...
class Simulation(Base):
    ...
    def main(self):
        ...
        # 証券コード
        for stock_code in config.STOCK_CODE:
            # 日付
            for target_date in config.TARGET_DATE:
                # DBから対象日のレコードを取得
                result, board_info = self.logic.select_boards(stock_code, target_date, config.START_TIME, config.END_TIME)
                if result == False:
                    continue
                ...
                for order_line in config.REORDER_LINE:
                    ...
                    for securing_benefit_border in config.SECURING_BENEFIT_BORDER:
                        ...
                        for loss_cut_border in config.LOSS_CUT_BORDER:
                            ...
                            price_range, unit_num = self.get_trade_width(stock_code)
                            # 設定情報を管理
                            setting_info = {
                                ...
                                'price_range': price_range,                # 呼値
                                'unit_num': unit_num                       # 売買単位
                            }
    ...
    def get_trade_width(self, stock_code):
        '''仮'''
        if stock_code == 1570:
            return 5, 1
        elif stock_code in [8411,7203]:
            return 1, 100
        elif stock_code == 8306:
            return 0.5, 100
        elif stock_code in [9432,4755,7201,9501]:
            return 0.1, 100
        else:
            raise
