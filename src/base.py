...
class Base():
    ...
    def create_instance(self, db = True, api = True):
        '''
        各クラスのインスタンスを生成する

        Args:
            db(bool): DBクラスを呼び出す/使用するか
                ※DBに接続されていないとエラーとなるので必要な場合のみTrueに

            api(bool): KabuStationAPIクラスを呼び出す/使用するか
        ...
        if api:
            self.api = KabusApi(log = self.log,
        ...
        # Serviceクラス
        self.service = Service(log = self.log,
                               util = self.util,
                               api = self.api,
                               db = self.db)
