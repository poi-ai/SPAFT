...
class RMSESignPenalty:
    def calc_ders_range(self, approxes, targets, weights):
        ...
        Args:
            ...
            weights(np.ndarray): サンプルの重み（ライブラリの都合で引数として送られるが使わない)
        ...
        for approx, target in zip(approxes, targets):
            error = approx - target
            # 符号が異なる場合にのみペナルティを10倍にする(RMSEのデフォルトの勾配は2、1にすると学習打ち切りになる)
...
def evaluation_rmse_sign_penalty(y_true, y_pred):
    ...
    penalty_factors = np.where(y_true * y_pred < 0, 10.0, 1.0)  # 符号が違う場合は10倍
