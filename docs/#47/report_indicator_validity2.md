# テクニカル指標 正当性チェックレポート（第2回: NaN統一修正後）

**生成日時:** 2026-04-06 11:30:06
**対象:** `src/util/indicator.py` の各テクニカル指標計算メソッド
**フェーズ:** issue #47 対応前の事前確認（NaN統一修正後の再チェック）

## 第1回からの変更点

`indicator.py` の以下3メソッドで、データ不足時（rolling windowが未充足）の
欠損値処理を `-1補完` から `NaN` に統一した。

| メソッド | 変更前 | 変更後 |
|---|---|---|
| `get_sma()` | `fillna(-1)` | NaNのまま（削除） |
| `get_ema()` | `fillna(-1)` | NaNのまま（削除） — ewmの性質上元々NaN発生なし |
| `get_wma()` | `fillna(-1)` | NaNのまま（削除） |

この変更によりMAクロスのdiff計算への汚染（`-1 - 有効値 = 株価絶対値レベルの外れ値`）
が解消されることを確認する。
...
**集計:** PASS=7 / WARNING=5 / FAIL=2 / ERROR=0
...
### 5. RSI・RCI・PSY の先頭行 NaN（NaN統一後も残る設計上の挙動）
